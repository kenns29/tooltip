...
module.exports = exports = function(_container) {
  ...
  function init() {
    tooltip = d3.select(container).append('div').attr('class', 'tooltip').style('z-index', z_index).style('opacity', 0).style('position', 'absolute').style('text-align', 'center').style('padding', '2px').style('font', '12px sans-serif').style('background', 'lightsteelblue').style('border', '0px').style('border-radius', '8px').style('display', 'none');
    return ret;
  }
  function x_pos(x) {
  ...
  ret.font_size = function(_) {
    return arguments.length > 0 ? (font_size = _, tooltip.style('font-size', (font_size + "px")), ret) : font_size;
  };
  ret.z_index = function(_) {
    return arguments.length > 0 ? (z_index = _, tooltip.style('z-index', z_index), ret) : z_index;
  };
  return ret;
};

//# sourceURL=/Volumes/SANDISK/tooltip/tooltip.js
},{"d3":1}]},{},[2]);
